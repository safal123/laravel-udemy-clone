...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ src, chapter, nextChapterId, previousChapterId, isCompleted }) => {
  ...
  const handleSliderHover = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Calculate thumbnail position with edge detection
    const thumbnailWidth = 160 // Width of thumbnail
    const halfThumbnailWidth = thumbnailWidth / 2

    let newPosition
    if (x < halfThumbnailWidth) {
      // At left edge
      newPosition = halfThumbnailWidth
    } else if (x > rect.width - halfThumbnailWidth) {
      // At right edge
      newPosition = rect.width - halfThumbnailWidth
    } else {
      // Normal positioning
      newPosition = x
    }

    setThumbnailPos(newPosition)
    ...
    >
      ...
      <div className="h-full">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4 pt-20">
          ...
          <div
            ref={progressBarRef}
            className="relative w-full h-3 group/progress cursor-pointer mb-2 hover:h-4 transition-all"
          ...
          >
            ...
            {isHovering && (
              <div
                className="absolute bottom-full mb-3 bg-black/90 rounded-md overflow-hidden transform -translate-x-1/2 shadow-xl border border-white/10"
            ...
            {/* Progress bar */}
            <div className="absolute bottom-0 left-0 right-0 h-1 group-hover/progress:h-full transition-all bg-white/20 rounded-full overflow-hidden">
              {/* Buffered progress */}
              <div
              ...
              >
                {/* Thumb */}
                <div className="absolute right-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 bg-red-600 rounded-full scale-0 group-hover/progress:scale-100 transition-transform shadow-md border-2 border-white" />
              </div>
            </div>
          </div>
