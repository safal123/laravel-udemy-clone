...
const ReactVideoPlayer = ({ src, chapter, isCompleted }: ReactVideoPlayerProps) => {
  ...
  const getThumbnailStyles = (position: number | null): ThumbnailStyle => {
    if (!position) {
      return {
        left: 0,
        transform: 'none',
        ...
        <div className="relative" style={{ height: isHovering ? '100px' : '10px' }}>
          ...
          {isHovering && thumbnailUrl && (
            <div
              ...
              style={getThumbnailStyles(hoverPosition)}
            >
              <img
                src={thumbnailUrl}
                alt="Preview"
              ...
              />
              <div className="absolute bottom-0 left-0 right-0 py-1.5 px-2 text-xs text-white text-center bg-black/90 font-medium">
