...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ src, chapter, nextChapterId, previousChapterId, isCompleted }) => {
  ...
  const handleSliderHover = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Adjust position at edges
    if (x < thumbnailWidth / 2) {
      newPosition = 0 // Align to left edge
    } else if (x > rect.width - thumbnailWidth / 2) {
      newPosition = rect.width // Align to right edge
    }
    ...
    >
      ...
      <div className="h-full">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4 pt-20">
          ...
          <div
            ref={progressBarRef}
            className="relative w-full h-1 group/progress cursor-pointer mb-2"
          ...
          >
            ...
            {isHovering && (
              <div
                ...
                style={{
                  left: thumbnailPos <= 80 ? 0 :
                    thumbnailPos >= progressBarRef.current!.getBoundingClientRect().width - 80 ?
                      'auto' :
                      `${thumbnailPos}px`,
                  right: thumbnailPos >= progressBarRef.current!.getBoundingClientRect().width - 80 ?
                    0 : 'auto',
